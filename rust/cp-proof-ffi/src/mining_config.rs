...
mod tests {
    ...
    fn bzminer_patterns_encode() {
        ...
        let cfg = mining_config_bytes(4096, 256, &rows, &cols).expect("bzminer cfg");
    ...
    fn contiguous_config_bytes_match_embedded() {
        ...
        let cfg = mining_config_bytes(4096, 256, &rows, &cols).expect("contiguous cfg");
        const EMBEDDED: [u8; 52] = [
            0x00, 0x10, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x07, 0x00, 0x00, 0x00, 0x00,
    ...
    fn cutlass_config_bytes_match_embedded() {
        ...
        let cols: Vec<u32> = vec![0, 32, 64, 96, 128, 160, 192, 224];
        let cfg = mining_config_bytes(4096, 256, &rows, &cols).expect("cutlass cfg");
        const EMBEDDED: [u8; 52] = [
            0x00, 0x10, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x03, 0x01, 0x03, 0x00, 0x00,
            0x1f, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ];
        assert_eq!(cfg, EMBEDDED);
