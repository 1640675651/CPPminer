...
/* pearl_mining.MiningConfiguration rows=[0..15] cols=[0..15] k=4096 r=128 */
...
void pearl_set_contiguous_tile_shape(int mr, int w){
    if(mr == 4)
        g_pearl_contiguous_tile_mr = 4;
    else if(mr == 16)
        g_pearl_contiguous_tile_mr = 16;
    else
        g_pearl_contiguous_tile_mr = 8;
    g_pearl_contiguous_tile_w = (w == 8) ? 8 : 16;
}
...
static const uint8_t* pearl_active_mining_config(void){
    ...
    if(g_pearl_contiguous_tiles) {
        ...
        if(g_pearl_contiguous_tile_mr == 16 && g_pearl_contiguous_tile_w == 16)
