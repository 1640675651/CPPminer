...
static const uint64_t k_wgpu_search_chunk = 1000000ull;
...

{
    ...
    while(stop_rc == CP_JOB_NONE){
        ...
        const int st = cp_wgpu_worker_search(
            ...
            job->mining_hash, diff, job->target, cur, k_wgpu_search_chunk,
        ...
        if(st == CP_WGPU_OK_FOUND){
            ...
            add_be_u64(cur, hashes > 0 ? hashes : k_wgpu_search_chunk);
