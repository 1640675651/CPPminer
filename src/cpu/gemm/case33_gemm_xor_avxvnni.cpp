...
/* Single-instruction replacement for maddubs+madd (u8×s8 → i32).
 * Use *_avx_* so MSVC emits VEX AVX-VNNI, not EVEX AVX512-VNNI. */
CASE33_FORCEINLINE __m256i rank4_dpbusd(__m256i acc, __m256i ua, __m256i sb) {
    return _mm256_dpbusd_avx_epi32(acc, ua, sb);
}
...
CASE33_FORCEINLINE void avxvnni_micro_gemm_kgroups(__m256i acc0[kColsPerGroup],
                                                   __m256i acc1[kColsPerGroup],
...
void avxvnni_micro_gemm_xor_fused_k_impl(const int8_t *a_base, const int8_t *b_base,
    ...
    if (use_fast_u8s8) {
        ...
        for (int kb = 0; kb < blocks_k; ++kb) {
            ...
            avxvnni_micro_gemm_kgroups(acc0, acc1, a_tile, b_tile,
...
void case33_avxvnni_micro_gemm_xor_fused_k(
    ...
    avxvnni_micro_gemm_xor_fused_k_impl(a_base, b_base, blocks_k, blocks_per_milestone,
