...
// --ocl-lds (CASE32_USE_LDS): stage LDS_KWG of packed A/B in __local (default KWG=16).
//   8×8 / KWG=16: CL_KERNEL_LOCAL_MEM_SIZE ≈ 4100 B/WG. Did not raise GT1 UMA scan rate.
// Scalar (no DP4A) inner loop: CLBlast GEMMK=0  cpm += aval * bscalar  (VWM=4, VWN=4).
//   default: float4 mad. --ocl-cpm-int (CASE32_CPM_INT): int8 lanes, int4 acc (not char:
//   |a*b|<=16129). --dev 8×8 float ~9.9 GMAC/s; production 4×8 float ~28 GMAC/s (GT1).
